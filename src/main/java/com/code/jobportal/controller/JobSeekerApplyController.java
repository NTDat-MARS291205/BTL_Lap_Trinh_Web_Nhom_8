...
import com.code.jobportal.entity.*;
import com.code.jobportal.services.*;
import com.code.jobportal.entity.*;
import com.code.jobportal.services.*;
...
//        Gọi service lấy chi tiết job theo id
        JobPostActivity jobDetails = jobPostActivityService.getOne(id);

//        Lấy danh sách ứng viên (đã apply) và danh sách người đã lưu job này — phục vụ kiểm tra trạng thái
...
//        Thông tin đăng nhập
        ...
        if (!(authentication instanceof AnonymousAuthenticationToken)) {
            if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))) {
                ...
                if (user != null) {
                    boolean exists = false; // Trạng thái apply
                    boolean saved = false;  // Trạng thái save
